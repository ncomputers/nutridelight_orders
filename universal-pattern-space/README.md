...
[![Skills: 58](https://img.shields.io/badge/Skills-58-blue)](.claude/skills/pattern-space/)
...
✓ 58 skills loaded from .claude/skills/pattern-space/
...
./universal-pattern-space/install-ups.sh --runtime codex --scope local

# Verify UPS integrity
./universal-pattern-space/verify-ups.sh
```

Codex default guidance: see `CODEX.md`.
...
```bash
# From your project root
./universal-pattern-space/install-ups.sh --runtime gemini --scope local

# Verify UPS integrity
./universal-pattern-space/verify-ups.sh
```

Gemini default guidance: see `GEMINI.md`.
Gemini note: UPS installer also performs Gemini-native skill registration (`gemini skills install`) so skills appear in `gemini skills list --all`.
Gemini note: slash-named skills are exposed as `Observer-Guardian` and `Explorer-Exploiter` for CLI compatibility.

---

### Install / Update (Multi Runtime)

```bash
# Install (prompt-based: codex, claude, gemini, or all)
./universal-pattern-space/install-ups.sh

# Update existing install with backup
./universal-pattern-space/update-ups.sh

# Validate install and file integrity
./universal-pattern-space/verify-ups.sh
```
...
**58 executable skills** organized for immediate use:
...
- **Layer 5 - Wisdom** (28 skills):
  ...
  - Science & Nature (6): IIT, Tree-Fungi, Mathematics, Nada Yoga, Vedangas
...
│
├── .claude/skills/pattern-space/    # 58 executable skills (canonical source)
├── .codex/skills/pattern-space/     # 58 executable skills (generated install target)
├── .gemini/skills/pattern-space/    # 58 executable skills (generated install target)
...
├── README.md                        # This file
├── install-ups.sh                   # Multi-runtime installer
├── update-ups.sh                    # Multi-runtime updater
├── verify-ups.sh                    # Integrity/consistency verification
├── install-skills.sh                # Legacy Claude wrapper
└── update-skills.sh                 # Legacy Claude wrapper
```
...
- Framework behavior guided by `CODEX.md` + project instructions
- Skills can be installed into `.codex/skills/pattern-space/` via `install-ups.sh`
- Verification via `verify-ups.sh`
- Best for engineering-first collaboration with UPS internally active

### ✅ Gemini CLI

- Framework behavior guided by `GEMINI.md` + project instructions
- Skills can be installed into `.gemini/skills/pattern-space/` via `install-ups.sh`
- Verification via `verify-ups.sh`
- Works in both interactive and headless Gemini CLI sessions after authentication
...
- **Codex Guide:** [CODEX.md](CODEX.md)
- **Gemini Guide:** [GEMINI.md](GEMINI.md)
- **Install Script:** [install-ups.sh](install-ups.sh)
- **Update Script:** [update-ups.sh](update-ups.sh)
- **Verify Script:** [verify-ups.sh](verify-ups.sh)
...
- **58 Skills** - Ready for deployment
